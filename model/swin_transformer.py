...
model_name = {
    ...
    'swin_transformer_large': 'swin_large_patch4_window7_224_22kto1k.pth'
}
...
def swin_transformer_large(pretrained=True, **kwargs):
    """VGG 16-layer model (configuration "D") with batch normalization
    ...
    model = SwinTransformer(embed_dim=192, depths=[ 2, 2, 18, 2 ], num_heads=[6, 12, 24, 48],
                 window_size=7, drop_path_rate=0.2, **kwargs)
    if pretrained:
        model.load_state_dict(torch.load(os.path.join(models_dir, model_name['swin_transformer_large']))['model'])
    return model
